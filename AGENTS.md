...
- Supported upstream capabilities should be transparently forwarded. Known unsupported capabilities that have a clear counterpart in the selected upstream API should fail locally or surface a clean upstream-aligned unsupported error instead of being silently dropped or rerouted into a false success. For cross-protocol compatibility gaps with no upstream counterpart, keep the existing usable translation path unless there is evidence that the lossy behavior breaks real clients.
